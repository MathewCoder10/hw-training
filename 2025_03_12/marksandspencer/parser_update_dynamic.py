...
cursor = crawler_collection.find({})

for document in cursor:
    # Retrieve product_url from the document
    ...
    # Extract the color parameter (if exists) and rebuild the URL without query parameters
    ...
    # Set up request headers (adjust as needed)
    headers = {
    ...
    # Prepare parameters with the dynamic color parameter
    ...
    if response.status_code != 200:
        print(f"Non-200 response for URL {base_url}: {response.status_code}")
        continue
    ...
    if not script_content:
        print(f"No __NEXT_DATA__ script found for URL: {base_url}")
        continue

    try:
        data = json.loads(script_content)
    except json.JSONDecodeError as e:
        print(f"JSON decode error for URL {base_url}: {e}")
        continue
    ...
    # Output the scraped details for the current product.
