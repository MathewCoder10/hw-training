...
class Parser:
    def __init__(self):
        ...
        self.parser_collection = self.db['parser_8']
    ...
    def parse_items(self, document):
        ...
        print("Using color parameter:", color_param)
        
        # Set referer dynamically.
        self.scrape_headers['referer'] = full_product_url
        
        try:
            # If color_param is provided, pass it as a parameter; otherwise, request without it.
            if color_param is not None:
                params = {'color': color_param}
                response = requests.get(pdp_url, params=params, headers=self.scrape_headers, timeout=10)
            else:
                response = requests.get(pdp_url, headers=self.scrape_headers, timeout=10)
        ...
        common_details["care_instructions"] = selector.xpath("//p[text()='Care']/following-sibling::div//p[@class='media-0_textSm__Q52Mz product-details_careText__t_RPG']/text()").getall()
        ...
        # --- Extract the selected color from the page ---
    ...
    "experience": {{"id": "treatment-v9"}}
        ...
        if variants_data:
            ...
            for variant in variants_data:
                for sku in variant.get("skus", []):
                    # Safely extract size dictionary.
                    size_info = sku.get("size") or {}
                    primary = size_info.get("primarySize")
                    secondary = size_info.get("secondarySize")
                    if primary:
        ...
        for variant in variants_data:
            ...
            for sku in variant.get("skus", []):
                ...
                # Add the new selected_color field.
                flat_record["selected_color"] = selected_color
                
                # Add SKU-specific fields.
                flat_record["sku_id"] = sku.get("id")
                size_info = sku.get("size") or {}
                primary_size = size_info.get("primarySize")
                secondary_size = size_info.get("secondarySize")
