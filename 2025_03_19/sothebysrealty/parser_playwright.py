...
from playwright.sync_api import sync_playwright  # Added Playwright import
...
def convert_language_in_url(url, target_lang="eng"):
    """
    Converts the language segment in the URL path to the target_lang.
    Example:
      https://www.sothebysrealty.com/zhs/associate/...
      becomes
      https://www.sothebysrealty.com/eng/associate/...
    """
    parsed = urlparse(url)
    segments = parsed.path.split('/')
    # Expecting the first segment to be empty and the second to be the language code
    if len(segments) > 1 and segments[1] != target_lang:
        segments[1] = target_lang
...
class Parser:
    ...
    def __init__(self):
        # Set up cookies and headers with an initial aws-waf-token placeholder.
        self.cookies = {
            ...
            'aws-waf-token': 'initial_token',  # Initial placeholder token
        ...
        self.db = self.client['sotheby_db']
        # The 'crawler' collection stores the URLs to be processed
        self.crawler_collection = self.db['agent_crawler']
    ...
    def fetch_url(self, url, retries=3):
        """Fetch a URL with a retry mechanism for 400, 500 and 202 based responses."""
        attempt = 0
        while attempt < retries:
            try:
                response = requests.get(url, headers=self.headers, cookies=self.cookies)
                # If response status code is 202, update the aws-waf-token dynamically
                if response.status_code == 202:
                    logging.info(f"Received 202 for {url}. Generating new aws-waf-token.")
                    try:
                        new_token = fetch_new_aws_waf_token(url)
                        self.cookies['aws-waf-token'] = new_token
                        logging.info("Updated aws-waf-token. Retrying request...")
                    except Exception as e:
                        logging.error(f"Error fetching new aws-waf-token: {e}")
                # Check if the response is OK
                elif response.status_code == 200:
                    return response
                # For 400 or 500 errors, log and retry
                elif 400 <= response.status_code < 600:
                    logging.error(f"Error fetching {url}: status code {response.status_code}. Retrying attempt {attempt + 1} of {retries}...")
            except Exception as e:
                logging.error(f"Exception fetching {url}: {e}. Retrying attempt {attempt + 1} of {retries}...")
            attempt += 1
    ...
    def start(self):
        ...
        for meta in metas:
            ...
            # Convert language in the URL to "eng" if necessary.
            url = convert_language_in_url(url, target_lang="eng")
            ...
            # Introduce a 1 second delay between a random number of requests.
